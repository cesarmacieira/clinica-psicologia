...
# Função para autenticação de senha
def check_password():
    if 'password_correct' not in st.session_state:
        # Inicializa o estado da senha como não autenticada
        st.session_state.password_correct = False
    
    # Se a senha já estiver correta, mostra o app
    ...
    if st.button('Entrar'):
        if password == '':
            st.info('Por favor, digite a senha.')
            return False
        elif password == '13':  # Substitua '13' pela senha que você deseja
...
def generate_pdf(nome, cpf, valor, data_extensa):
    ...
    marca_dagua = Image.open("Marca dagua.png")
...
st.title('Gerador de recibos - Psicóloga Janine Viana')

# Verifica se a senha está correta antes de mostrar o app
if check_password():
    ...
    if st.button('Gerar recibo'):
        if not nome:
            st.error("O campo 'Nome do paciente:' não pode ficar vazio.")
        elif not cpf:
            st.error("O campo 'CPF do paciente:' não pode ficar vazio.")
        elif len(cpf) != 11 or not cpf.isdigit():
            st.error("O CPF deve conter exatamente 11 dígitos.")
        elif valor == 0:
            st.error("O valor da sessão não pode ser zero.")
        elif not data_input:
            st.error("O campo 'Data' não pode ficar vazio.")
        else:
            # Adiciona o spinner enquanto o recibo é gerado
            with st.spinner('Gerando recibo...'):
                data_extensa = data_extenso(data_input)
                buffer = generate_pdf(nome, cpf, valor, data_extensa)
                download_pdf(buffer, f"Recibo {nome}  {data_num}.pdf")
